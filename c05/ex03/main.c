#include <stdio.h> // Para usar a funcao printf

// Prototipo da sua funcao ft_recursive_power
int	ft_recursive_power(int nb, int power);

int main()
{
	printf("--- INICIO DOS TESTES DE ft_recursive_power ---\n\n");
	...
	printf("-5^0 (Esperado: 1)     -> Resultado: %d\n", ft_recursive_power(-5, 0));
	printf("0^0 (Esperado: 1)      -> Resultado: %d\n", ft_recursive_power(0, 0)); // Caso especial do subject
	...
	printf("\n### Testes de Overflow (Retorno Indefinido) ###\n");
	printf("2^30 (Esperado: Overflow/Indefinido) -> Resultado: %d\n", ft_recursive_power(2, 30)); // 2^30 = 1,073,741,824 (cabe)
	printf("2^31 (Esperado: Overflow/Indefinido) -> Resultado: %d\n", ft_recursive_power(2, 31)); // 2^31 = 2,147,483,648 (estoura INT_MAX)
	printf("3^20 (Esperado: Overflow/Indefinido) -> Resultado: %d\n", ft_recursive_power(3, 20)); // 3^20 = 3,486,784,401

	printf("\n--- FIM DOS TESTES DE ft_recursive_power ---\n");
	return (0);
}
